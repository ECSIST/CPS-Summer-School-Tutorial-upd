...
A Python 3.12 / Linux x86_64 environment for `CPS Tutorial - Concrete.ipynb`, including the supplied ONNX model and test data.
...
Core versions: Concrete ML **1.9.0**, Concrete Python **2.10.0**, NumPy **1.26.4**, scikit-learn **1.5.0**, ONNX **1.17.0**, ONNX Runtime **1.18.0**, and CPU-only PyTorch **2.3.1+cpu**. Notebook support uses ipykernel **6.29.5** and plotting uses Matplotlib **3.9.2**. `time` is part of Python's standard library.
...
The replacement notebook is based on the supplied `CPS Tutorial - Concrete.ipynb`, including its revised teaching text and two-bit quantization setting. Its pip installation cells were replaced with version checks, including the ONNX 1.23.0 installation cell, because Concrete ML 1.9.0 requires ONNX 1.17.0. The arithmetic example now creates a seeded input set with explicit range endpoints instead of relying on `fhe.inputset`. Teaching exercises and the original encrypted execution settings remain intact.
